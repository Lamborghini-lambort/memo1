...
import com.agent.worker.agent.tool.McpToolAdapter;
...
@Slf4j
public class IntentAgent extends LLMDrivenReActAgent {

    private final McpClient mcpClient;
    private final McpToolAdapter mcpToolAdapter;

    public IntentAgent(LLMReActService llmReActService,
    ...
    protected void registerTools() {
        // 注册 MCP 工具适配器
        try {
            log.info("[IntentAgent] 正在从 MCP Server 获取工具列表...");
            JSONObject toolsInfo = mcpClient.listTools();
            
            var tools = toolsInfo.getJSONArray("tools");
            if (tools != null && !tools.isEmpty()) {
                for (int i = 0; i < tools.size(); i++) {
                    JSONObject toolInfo = tools.getJSONObject(i);
                    String toolName = toolInfo.getString("name");
                    String description = toolInfo.getString("description");
                    
                    log.info("[IntentAgent] 发现 MCP 工具：{} - {}", toolName, description);
                    
                    // 创建 MCP 工具适配器实例
                    McpToolAdapter adapter = new McpToolAdapter(mcpClient);
                    adapter.setToolInfo(toolName, description, "动态参数");
                    
                    toolRegistry.register(adapter);
                }
                log.info("[IntentAgent] 成功注册 {} 个 MCP 工具", tools.size());
            }
        } catch (Exception e) {
            log.error("[IntentAgent] 获取 MCP 工具失败：{}", e.getMessage(), e);
            throw new RuntimeException("无法连接 MCP Server，启动失败", e);
        }
        
        log.info("[IntentAgent] 总共注册了 {} 个工具", toolRegistry.getAllTools().size());
    }
    ...
    public Result<String> analyzeIntentV2(@RequestBody JSONObject params) {
        ...
        log.info("[IntentAgent] 收到意图分析请求，taskId={}", taskId);
